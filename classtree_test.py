...
def test_subclasses_of_type():
    ...
    is an unbound method when called on it, so we call it as 
    ...
    subclass of `type` (i.e metaclass)  I we get when I run `$ classtree.py type` at the 
    commnad line.

    However, the Python console and `pytest` both load other modules, 
    sp `subclasses` may find more subclasses of `type`- for eg:
    `enum.EnumMeta`
...
def test_render_lines_4_levels_leaves_dedent():
    class A: pass 
    class B1(A): pass 
    class C1(B1): pass 
    class D1(C1): pass 
...
def test_main_simple(capsys):
    ...
    expected = dedent("""
        ...
        └── TabError
    """).lstrip()
    captued = capsys.readouterr()
    assert captued.out == expected
